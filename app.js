...
function showEvolutionAnimation(result){
  const rec=getStudentRecord(result.student);
  const displayName=getShortMonsterName(rec,result.monster);
  const fromLevel=Math.max(1,(result.evolved_to||2)-1);
  const toLevel=result.evolved_to||2;
  const oldSrc=monsterImagePath(result.monster,fromLevel);
  const newSrc=monsterImagePath(result.monster,toLevel);
  clearTimers();
  document.removeEventListener("keydown",battleKeyHandler);
  setScreen(`<div class="evolution-screen">
    <div class="evo-title">${escapeHtml(displayName)} er að þróast!</div>
    <div class="evo-stage">
      <img id="evoOldImg" class="evo-monster evo-old" src="${oldSrc}" alt="${escapeHtml(displayName)}">
      <img id="evoNewImg" class="evo-monster evo-new" src="${newSrc}" alt="${escapeHtml(displayName)}">
      <div class="evo-glow"></div>
    </div>
    <div id="evoMessage" class="evo-message">Hvað er að gerast?</div>
    <button id="evoContinue" class="btn-green evo-continue" style="display:none" onclick="showResult(window.pendingEvolutionResult)">Halda áfram</button>
  </div>`);
  window.pendingEvolutionResult=result;
  const oldImg=document.getElementById("evoOldImg");
  const newImg=document.getElementById("evoNewImg");
  const msg=document.getElementById("evoMessage");
  const btn=document.getElementById("evoContinue");
  oldImg.onerror=()=>{oldImg.outerHTML='<div class="missing-img">Mynd vantar:<br>'+oldSrc+'</div>'};
  newImg.onerror=()=>{newImg.outerHTML='<div class="missing-img">Mynd vantar:<br>'+newSrc+'</div>'};
  let steps=0;
  const maxSteps=42;
  const tick=()=>{
    steps++;
    const progress=steps/maxSteps;
    const showNew=steps%2===0;
    if(oldImg&&newImg){
      oldImg.style.opacity=showNew?0:1;
      newImg.style.opacity=showNew?1:0;
    }
    let delay=Math.max(70,420-(progress*360));
    if(steps<maxSteps){
      ...
      if(oldImg&&newImg){
        ...
        newImg.classList.add("evo-final");
      }
      msg.textContent=`Til hamingju! ${displayName} hefur þróast!`;
      btn.style.display="inline-block";
      setTimeout(()=>{if(window.pendingEvolutionResult===result)showResult(result)},4500);
    }
  };
  setTimeout(tick,800);
}
...
function finishBattle(won){if(!battle||battle.finished)return;battle.finished=true;document.removeEventListener("keydown",battleKeyHandler);clearTimers();const result=buildResult(won);const evo=recordMonsterBattleAndEvolve(battle.monster);result.evolved_to=evo.evolved_to;result.monster_level_after=evo.level;result.monster_battles_at_level_after=evo.battles_at_level;result.monster_total_battles_after=evo.total_battles;result.gold_earned=calculateGold(result,won);result.gold_difficulty_multiplier=result.difficulty_mode==="tables"?1:(DIFFICULTY_GOLD_MULTIPLIER[result.difficulty_label]||1);result.gold_operation_multiplier=OPERATION_GOLD_MULTIPLIER[result.operation_label]||1;const rec=getStudentRecord(battle.student);rec.gold+=result.gold_earned;rec.sessions.push(result);saveData();if(result.evolved_to){showEvolutionAnimation(result)}else{showResult(result)}}
